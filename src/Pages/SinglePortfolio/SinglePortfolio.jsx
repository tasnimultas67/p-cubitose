            ...
            <div className="w-10/12 m-auto">
                ...
                <div>
                    <motion.div
                        ...
                        <div className="relative bg-gradient-to-t from-black/80 to-black/25 pt-28 pb-4 sm:pt-44 sm:pb-5 lg:pt-60 lg:pb-6">
                            <p className="block text-sm text-white/90 backdrop-blur-lg bg-white/10 w-fit px-2 py-1 rounded-md absolute top-5 left-5">{ portfolioServiceCategory=="Web-Development"? "Web Development": portfolioServiceCategory=="Graphics-Design"? "Graphics Design": portfolioServiceCategory=="Digital-Marketing"?"Digital Marketing": portfolioServiceCategory=="Web-Design"?"Web Design": portfolioServiceCategory}</p>
                                ...
                                {portfolioClientName?.length > 2 && <p className="text-gray-100">Client: { portfolioClientName}</p>}
                                {portfolioCountry?.length > 2 && <p className="text-gray-100">Country: { portfolioCountry}</p>}
                                    <p className="text-gray-100 mb-3 mt-2">{ portfolioShortDescription}</p>
                                    <Link className="text-white flex items-center w-fit" to={portfolioLink} target="_blank">{portfolioLink} <ArrowTopRightOnSquareIcon className="h-4 w-4 ml-2 text-white"/></Link>
                                </div>
                            </div>
                    </motion.div>
                </div>
                ...
                {portfolioDetails?.length > 0 ? <motion.div
                    ...
                    <motion.div
                        ...
                        viewport={{ once: true }}
                        className="text-black space-y-5 text-justify mt-10">
                    ...
                    <img className="w-full h-[25rem] object-cover rounded-md shadow-md" src="https://ameceg.com/wp-content/uploads/2020/07/000000.jpg" alt="Dummy Image" />
